...
contract UniswapRouter2 is IUniswapRouter, IERC721Receiver {
    ...
    // ToDo: transfer ownership to mainston
    constructor(
    ...
    function getTokensToMint(address _transitionInterfaceAddress) internal returns (int256 amount) {
        IMarketTransition transition = IMarketTransition(_transitionInterfaceAddress);
    ...
    function decreaseLiquidity(uint256 tokenId, uint128 liquidity) external returns (uint amount0, uint amount1) {
        ...
        METADEX.transfer(msg.sender, amount1);
        ...
        )
    {
        ...
        TransferHelper.safeTransferFrom(address(USDC), msg.sender, address(this), amountAdd0);
        TransferHelper.safeTransferFrom(address(METADEX), msg.sender, address(this), amountAdd1);

        TransferHelper.safeApprove(address(USDC), address(manager), amountAdd0);
        TransferHelper.safeApprove(address(METADEX), address(manager), amountAdd1);
        ...
        if(amountAdd0 > amount0) {
            ...
            TransferHelper.safeTransfer(address(USDC), msg.sender, diff);
        }

        if(amountAdd1 > amount1) {
            uint256 diff2 = amountAdd1 - amount1;
            TransferHelper.safeTransfer(address(METADEX), msg.sender, diff2);
        }
    }
    ...
    /// @notice Swaps a fixed amount of USDC for a maximum possible amount of METADEX using the USDC/METADEX 0.3% pool by calling `exactInputSingle` in the swap router.
    ...
    /// @param _amountIn The exact amount of USDC that will be swapped for METADEX.
    /// @return amountOut The amount of METADEX received.
    function swapUSDCForMETADEX(uint256 _amountIn) external isActive returns (uint256 amountOut) {
        require(_amountIn > 0, "Amount must be larger than zero");

        TransferHelper.safeTransferFrom(address(USDC), msg.sender, address(this), _amountIn);

        TransferHelper.safeApprove(address(USDC), address(swapRouter), _amountIn);

        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            tokenIn: address(USDC),
            tokenOut: address(METADEX),
            fee: poolFee,
        ...
        emit TokensSwapped(address(USDC), address(METADEX), _amountIn, amountOut);
