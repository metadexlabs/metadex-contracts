...
import "./interfaces/IMetaDexLottery.sol";

/** @title MetaDex Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract MetaDexLottery is ReentrancyGuard, IMetaDexLottery, Ownable {
    ...
    uint256 public maxPriceTicketInMetadex = 50 ether;
    uint256 public minPriceTicketInMetadex = 0.005 ether;
     ...
     * @param _metadexTokenAddress: address of the METADEX token
        ...
        nonReentrant
    {
        ...
        // Calculate number of METADEX to this contract
        uint256 amountMetadexToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInMetadex,
            _ticketNumbers.length
        );

        // Transfer METADEX tokens to this contract
        metadexToken.safeTransferFrom(address(msg.sender), address(this), amountMetadexToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMetadex += amountMetadexToTransfer;
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            _numberTicketsPerLotteryId[_lotteryId][111 + ((thisTicketNumber / 1000) % 1000)]++;
            _numberTicketsPerLotteryId[_lotteryId][1111 + ((thisTicketNumber / 100) % 10000)]++;
            _numberTicketsPerLotteryId[_lotteryId][11111 + ((thisTicketNumber / 10) % 100000)]++;
            _numberTicketsPerLotteryId[_lotteryId][111111 + ((thisTicketNumber / 1) % 1000000)]++;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInMetadexToTransfer
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMetadexToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        metadexToken.safeTransfer(msg.sender, rewardInMetadexToTransfer);

        emit TicketsClaim(msg.sender, rewardInMetadexToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in METADEX per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMetadex) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].metadexPerBracket[j] =
                ...
                // A. No METADEX to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].metadexPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInMetadex - amountToShareToWinners);

        // Transfer METADEX to treasury address
        metadexToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in METADEX token
     ...
     * @param _priceTicketInMetadex: price of a ticket in METADEX
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInMetadex: _priceTicketInMetadex,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            metadexPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInMetadex: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    /**
     * @notice Set METADEX price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInMetadex: minimum price of a ticket in METADEX
     * @param _maxPriceTicketInMetadex: maximum price of a ticket in METADEX
     */
    function setMinAndMaxTicketPriceInMetadex(uint256 _minPriceTicketInMetadex, uint256 _maxPriceTicketInMetadex)
        external
        onlyOwner
    {
     ...
     * @param _priceTicket price of a ticket (in METADEX)
    ...
    function _calculateRewardsForTicketId(
        ...
        uint32 transformedWinningNumber = _bracketCalculator[_bracket] + 
            ((winningTicketNumber / (100000  / (uint32(10)**_bracket)) ) % (uint32(10)**(_bracket + 1)));

        uint32 transformedUserNumber = _bracketCalculator[_bracket] + ((userNumber / (100000  / (uint32(10)**_bracket)) ) % (uint32(10)**(_bracket + 1)));
        
        // Confirm that the two transformed numbers are the same, if not throw
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].metadexPerBracket[_bracket];
        } else {
            return 0;
        }
    }
